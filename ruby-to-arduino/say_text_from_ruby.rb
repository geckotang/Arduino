...
def readableName (screen_name, name)
  name = screen_name

  pp screen_name.to_hiragana
  pp name.to_hiragana
...
client.user do |status|
  if status.has_key? "event"
    if status.target.screen_name == "GeckoTang"
      readableName(status.source.screen_name, status.source.name)
      case status.event.to_sym
      when :favorite
        # ファボ
        s_name = status.source.screen_name
        sp.puts "okiniirini/irema'_shita."
      when :unfavorite
        # アンファボ
        s_name = status.source.screen_name
        sp.puts "okiniirikara/hazusarema'_shita."
      else
        # 実装されていないイベント
      end
    end
    ...
    if Moji.downcase(status.text).index('geckotang')
      sp.puts "me'nnshonnga/todokima'_shita."
    end
  end
end
sp.close
